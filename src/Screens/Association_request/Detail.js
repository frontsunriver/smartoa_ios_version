...
import { Platform, View, Text, StyleSheet, TextInput, Image, Keyboard, PermissionsAndroid, Dimensions } from 'react-native';
...
const Detail = ({route, navigation}) => {
  ...
  const downloadIcon = parseIconFromClassName('fas fa-eye')
  ...
  const fileDonwload = (url) => {
    ...
    let options = {
      ...
      config(options).fetch('GET', `${DOWNLOAD_URL}` + url).then((res) => {
          navigation.navigate(Routes.SHOW_VIEW_PDF_SCREEN, {data: res.path()})
  ...
  const showFile = async (url) => {
    ...
    if(extension.toLowerCase() == 'pdf') {
        // fileDonwload(url);
        navigation.navigate(Routes.SHOW_VIEW_PDF_SCREEN, {data: DOWNLOAD_URL + url})
    } else if(extension.toLowerCase() == 'jpg' || extension.toLowerCase() == 'jpeg' || extension.toLowerCase() == 'bmp' || extension.toLowerCase() == 'png') {
        await setImage([
            {
                source: {
                    uri: DOWNLOAD_URL + url,
                },
                width: width - 50,
                height: height - 100,
            },
